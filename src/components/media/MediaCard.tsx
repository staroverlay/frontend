...
export const MediaCard: React.FC<MediaCardProps> = ({ upload, onDelete, onSelect, formatBytes, isSelected }) => {
    ...
    return (
        <div
            className={`group flex flex-col rounded-2xl bg-zinc-900 border transition-all hover:shadow-xl hover:shadow-black/40 overflow-hidden ${isSelected ? 'border-violet-500 ring-2 ring-violet-500/20' : 'border-white/5 hover:border-violet-500/30'
                }`}
        ...
        >
            <div className={`aspect-square bg-zinc-950 relative overflow-hidden ${onSelect ? 'cursor-pointer' : ''}`}>
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center gap-3 p-4 backdrop-blur-[2px]">
                    ...
                    {onSelect && (
                        <button
                            ...
                            className="px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 shadow-lg"
                        >
                            Select
                        </button>
                    )}
