    ...
    >
      <div className="absolute top-6 right-6">
        <Button 
           variant="ghost" 
           size="sm"
           className="h-9 px-3 text-zinc-400 hover:text-white hover:bg-zinc-800"
           onClick={onRefresh}
           disabled={isLoading}
        >
      ...
      <div className="flex flex-col gap-3 mt-4">
        <div className="p-4 rounded-xl border border-rose-500/20 bg-rose-500/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-2">
          ...
          <button 
            onClick={onRevokeAll}
            className="shrink-0 px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 border border-rose-500/20 rounded-lg text-xs font-semibold transition-colors"
          >
            Log out everywhere
          </button>
        </div>
        ...
        {sessions.map((session) => (
          <div 
            ...
            className={cn(
              ...
              session.current 
                ? "bg-violet-500/5 border-violet-500/20 shadow-sm" 
          ...
          >
            ...
            <div className="flex-1 min-w-0 space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {session.current && (
                  <span className="px-2 py-0.5 rounded text-violet-400 bg-violet-400/10 text-[10px] font-bold">
                    Current Device
                  </span>
                )}
              </div>
            ...
            <div className="w-full sm:w-auto mt-2 sm:mt-0">
              <button 
                className={cn(
                  "w-full sm:w-auto h-9 px-3 text-xs font-semibold rounded-lg transition-colors flex items-center justify-center gap-2",
                  session.current 
